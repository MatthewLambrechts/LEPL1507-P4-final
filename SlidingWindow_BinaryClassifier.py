...
# Donne différentes tailles de sous-images à é
def pyramid_2(image, scale=1.5, minSize=(16, 16)):
 	# yield the original image size (only non-square image)
...
#bonne version (version 2): les contours sont rétrécis, pas l'image 
#ancienne version (version 1): l'image rétrécit, pas les contours
...
# # Donne différentes tailles de sous-images à é
# def pyramid_2(image, scale=1.5, minSize=(5, 5)):
#  	# yield the original image size (only non-square image)
#     yield (image.shape[0],image.shape[1])
#     it = 0
#     #get minimum of height and width
...
# Y = []
# count = 0
# folders = sorted(os.listdir("challenge-2-non-cropped-images/eval_kaggle2_sorted"))
# for folder_path in folders:
#     label = int(folder_path)
#     images = sorted(os.listdir("challenge-2-non-cropped-images/eval_kaggle2_sorted/"+ folder_path), key=lambda f: int(f.split(".ppm")[0]))
#     for image_path in images :
#         image = cv2.imread("challenge-2-non-cropped-images/eval_kaggle2_sorted/" + folder_path + "/" + image_path)
#         X.append(image)
#         count += 1
#         Y.append(label)
...
#                 # Get predictions of pannel + background model (background class == 62)
#                 pannel_predictions = pannelModel.predict(np.expand_dims(img, axis=0))
#                 idx2 = np.argsort(pannel_predictions.flatten())[::-1][:3]
...
#         #Compare the result af pannel + background model to that of the cropped model
#         for i in range(len(probas_predictions)):
#             cropped_model_predictions = cropped_model.predict(np.expand_dims(kept_images[i], axis=0))
#             idx3 = np.argsort(cropped_model_predictions.flatten())[::-1][:3]
#             if(idx3[0] == class_predictions[i]) :
#                 found_same = True
